...
import { Building2, Globe, Loader2, Save, X } from "lucide-react";
...
interface Props {
  ...
  onClose: () => void;
  onSaved: (overview: Overview) => void;
}

export function OrgSettingsModal({ projectId, projectName, overview: initialOverview, onClose, onSaved }: Props) {
  const [isLoading, setIsLoading] = useState(!initialOverview);
  const [formData, setFormData] = useState<Overview>(initialOverview ?? {
    name: projectName,
    industry: "",
    size: "",
    regions: [],
  ...
  useEffect(() => {
    ...
    api.getProject(projectId).then((data) => {
      const overview = data.project.overview_json as Overview | null;
      if (overview) {
        setFormData({
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      await api.updateProjectOverview(projectId, formData as unknown as Record<string, unknown>);
      onSaved(formData);
      onClose();
    } catch (e) {
  ...
  const updateField = <K extends keyof Overview>(field: K, value: Overview[K]) => {
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative max-w-2xl w-full max-h-[90vh] bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-stripe-deep overflow-hidden flex flex-col"
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              ...
              <Section title="Basic Information">
                <FieldRow label="Organisation Name">
                  <input
                    ...
                    className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-md px-3 py-2 text-[13px] text-[var(--color-fg)] focus:outline-none focus:border-[var(--color-brand)] w-full"
                  />
                </FieldRow>
          ...
          <Section title="Location & Industry">
            <FieldRow label="Regions">
              <ChipSelect
                options={["Algeria", "France", "Tunisia", "Morocco", "Egypt", "Libya", "Mauritania"]}
                values={formData.regions ?? []}
                onChange={(vals) => updateField("regions", vals)}
              />
            </FieldRow>
            <FieldRow label="Languages">
              <ChipSelect
                options={["French", "Arabic", "English", "Spanish"]}
          ...
          <Section title="Systems & Processes">
            <FieldRow label="Business Systems">
              <ChipSelect
                options={["ERP", "CRM", "Accounting", "HRM", "Supply Chain", "None"]}
                values={formData.systems ?? []}
                onChange={(vals) => updateField("systems", vals)}
              />
            </FieldRow>
            <FieldRow label="Main Business Processes">
              <ChipSelect
                options={["Order Management", "Invoicing", "Inventory", "Procurement", "HR", "Customer Support", "Finance", "Marketing", "Sales", "None"]}
                values={formData.processes ?? []}
                onChange={(vals) => updateField("processes", vals)}
              />
            </FieldRow>
          </Section>
          ...
          <Section title="Compliance">
            <FieldRow label="Compliance Requirements">
              <ChipSelect
                options={["GDPR", "ISO27001", "SOC2", "HIPAA", "PCI-DSS", "None"]}
                values={formData.compliance ?? []}
                onChange={(vals) => updateField("compliance", vals)}
              />
            </FieldRow>
          </Section>
...
function ChipSelect({ options, values, onChange }: { options: string[]; values: string[]; onChange: (vals: string[]) => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1.5 items-center p-1.5 bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg min-h-[32px]">
        {values.filter(v => !options.includes(v)).map((val) => (
          <span
            key={val}
        ...
        <input
          ...
          className="flex-1 min-w-[120px] bg-transparent border-none outline-none text-[12px] text-[var(--color-fg)] placeholder:text-[var(--color-fg-subtle)]"
        />
      </div>
    </div>
  );
