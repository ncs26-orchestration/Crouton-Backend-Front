...
export interface CommandAction {
  ...
  label: string;
  hint?: string;
  group: "actions" | "setup" | "navigate" | "links";
...
const GROUP_LABEL: Record<CommandAction["group"], string> = {
  actions: "Actions",
  setup: "Setup",
  navigate: "Navigate",
  links: "Open",
...
export function CommandPalette({ open, onClose, commands }: Props) {
  ...
  const grouped = filtered.reduce<Record<string, CommandAction[]>>((acc, c) => {
    (acc[c.group] ||= []).push(c);
    return acc;
  }, {});
  const order: CommandAction["group"][] = ["actions", "setup", "navigate", "links"];
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-[520px] bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg shadow-stripe-deep overflow-hidden"
      ...
      >
        ...
        <div className="max-h-[46vh] overflow-y-auto nice-scroll py-1">
          {filtered.length === 0 && (
            <div className="px-4 py-8 text-center text-[12px] text-[var(--color-fg-subtle)]">
              No commands match <span className="font-mono">{query}</span>
            </div>
          )}
          {order.map((g) =>
            grouped[g] && grouped[g].length > 0 ? (
              <div key={g} className="pt-1 pb-0.5">
                <div
                ...
                {grouped[g].map((cmd) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full group flex items-center gap-2.5 mx-1 px-2.5 h-8 rounded text-left transition-colors ${
                        isActive
                          ? "bg-[var(--color-accent-bg)] text-[var(--color-fg)]"
                          : "text-[var(--color-fg)] hover:bg-[var(--color-surface-2)]"
                      }`}
                      style={{ fontWeight: 400, width: "calc(100% - 8px)" }}
                    >
                      {Icon ? (
                        <Icon
                          size={13}
                          strokeWidth={1.75}
                          className={
                            isActive ? "text-[var(--color-brand)]" : "text-[var(--color-fg-muted)]"
                          }
                        />
                      ) : (
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="inline-flex items-center justify-center min-w-[15px] h-[15px] px-1 font-mono text-[10px] border border-[var(--color-border)] rounded bg-[var(--color-surface)] text-[var(--color-fg-muted)]">
      {children}
    </kbd>
  );
}
