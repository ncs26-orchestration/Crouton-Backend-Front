...
import { Background, BackgroundVariant, Controls, MiniMap, ReactFlow, applyNodeChanges, useReactFlow, } from "@xyflow/react";
import { TaskNode } from "./nodes/TaskNode";
import { GatewayNode } from "./nodes/GatewayNode";
import { EventNode } from "./nodes/EventNode";
import { ConditionalEdge } from "./edges/ConditionalEdge";
import { irToFlow } from "../lib/ir-to-flow";
const nodeTypes = {
    task: TaskNode,
    gateway: GatewayNode,
    event: EventNode,
...
export function IRCanvas({ workflow, isRegistry, onTaskSelect, selectedTaskId, onEditFlowExpression, onClearFlowExpression, }) {
    const seed = useMemo(() => {
        if (!workflow)
            return { nodes: [], edges: [] };
    ...
    useEffect(() => {
        ...
        for (const n of seed.nodes) {
            ...
            if (n.type === "task" && was.type === "task") {
                const a = n.data;
                const b = was.data;
                if (a.bindingState !== b.bindingState ||
    ...
    useEffect(() => {
        if (!selectedTaskId || !workflow)
            return;
        const neighborIds = new Set([selectedTaskId]);
        for (const f of workflow.flows) {
            if (f.from === selectedTaskId)
                neighborIds.add(f.to);
            if (f.to === selectedTaskId)
                neighborIds.add(f.from);
        }
    ...
    if (!workflow) {
        return (<div className="w-full h-full flex items-center justify-center text-[var(--color-fg-subtle)] canvas-dots">
        <div className="max-w-md text-center px-6 anim-fade-in">
          <div className="mx-auto mb-4 size-12 rounded-2xl bg-[var(--color-accent-bg)] flex items-center justify-center">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--color-brand)]">
          ...
          <div className="text-[11px] tracking-widest uppercase text-[var(--color-fg-subtle)] mb-2" style={{ fontWeight: 400 }}>
            empty canvas
          </div>
    ...
    return (<ReactFlow nodes={selectedNodes} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onNodesChange={onNodesChange} fitView fitViewOptions={{ padding: 0.25 }} minZoom={0.3} maxZoom={1.5} proOptions={{ hideAttribution: true }} nodesDraggable nodesConnectable={false} elementsSelectable panOnDrag panOnScroll={false} zoomOnScroll onNodeClick={(_, node) => {
            if (!onTaskSelect)
                return;
            if (node.type === "task")
      ...
      <Background variant={BackgroundVariant.Dots} gap={22} size={1} className="!bg-[var(--color-bg)]"/>
      ...
      {nodes.length > 5 && (<MiniMap position="bottom-left" pannable zoomable ariaLabel="workflow minimap" nodeStrokeWidth={2} nodeBorderRadius={3} nodeColor={(n) => {
                const data = (n.data ?? {});
            ...
            }} maskColor="rgba(24,24,27,0.18)" style={{
                width: 160,
                height: 96,
