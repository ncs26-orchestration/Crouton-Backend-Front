...
export function TopBar({ workflow, status, target, adapters, onTargetChange, onCompile, onDeploy, deployBusy, compileDisabled, compileDisabledReason, lastCompiledBytes, taskCount, boundCount, extractionStage, interviewProgress, onOpenCopilot, }) {
    ...
    const selectorAdapters = adapters.length > 0 ? adapters : [{ kind: "camunda7", name: "Camunda 7" }];
    const tone = STATUS_TONE[status];
    const { active: pipelineActive, furthest: pipelineFurthest } = stageForStatus(status);
    return (<header className="h-14 shrink-0 bg-[var(--color-surface)] border-b border-[var(--color-border)] flex items-center px-5 gap-4">
      <div className="flex items-center gap-2 min-w-0">
        ...
        {extractionStage === "drafting" && interviewProgress ? (<button onClick={onOpenCopilot} className="group flex items-center gap-2 text-[10.5px] font-mono px-2 py-0.5 border rounded-full border-[var(--color-accent-border)] bg-[var(--color-accent-bg)] text-[var(--color-brand)] hover:brightness-110 transition-all" style={{ fontWeight: 500 }} title="Open Copilot to resolve the remaining questions">
          ...
          </button>) : (<span className={`flex items-center gap-1.5 text-[10px] font-mono px-2 py-0.5 border rounded-full ${tone.ring} ${tone.text}`}>
            <span className={`size-1.5 rounded-full ${tone.dot}`}/>
            {tone.label}
          </span>)}

        {taskCount !== undefined && taskCount > 0 && (<span className="text-[11px] text-[var(--color-fg-muted)] font-mono">
      ...
      {workflow && (<div className="hidden md:flex items-center ml-2">
          <PipelineChips active={pipelineActive} furthest={pipelineFurthest}/>
        </div>)}

      <div className="flex-1"/>

      {lastCompiledBytes ? (<span className="text-[11px] font-mono text-[var(--color-fg-subtle)]">
      ...
      <label className="flex items-center gap-1.5 text-xs text-[var(--color-fg-muted)]">
        ...
        <div className="relative">
          <select value={target} onChange={(e) => onTargetChange(e.target.value)} className="appearance-none bg-[var(--color-surface)] border border-[var(--color-border)] rounded-md pl-2.5 pr-7 py-1 text-xs text-[var(--color-fg)] hover:border-[var(--color-border-strong)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent-border)]">
            {selectorAdapters.map((a) => (<option key={a.kind} value={a.kind}>
                {a.name}
              </option>))}
          </select>
      ...
      <button onClick={onCompile} disabled={compileDisabled} title={compileDisabledReason ?? ""} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-fg)] hover:bg-[var(--color-surface-2)] disabled:opacity-40 disabled:cursor-not-allowed">
        <Download size={13}/>
        Compile
      </button>

      {onDeploy && (<button onClick={onDeploy} disabled={compileDisabled || deployBusy || !canDeploy} title={compileDisabledReason ??
                (canDeploy ? "" : `Deploy is not supported by the ${currentAdapter?.name ?? "selected"} adapter yet — use Compile to download the artifact.`)} className="relative flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-md text-white bg-[var(--color-accent)] hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed shadow-sm shadow-indigo-900/20">
