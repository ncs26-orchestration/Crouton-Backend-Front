...
export function EventNode({ data, selected }) {
    ...
    return (<div className="relative flex items-center justify-center cursor-grab active:cursor-grabbing" style={{ width: data.width, height: data.height }}>
      ...
      <div className={`rounded-full bg-[var(--color-surface)] flex items-center justify-center transition-all ${selected
            ? isStart
            ...
            : isStart
                ...
                : "border-[3px] border-[var(--color-fg)] shadow-stripe-ambient"}`} style={{ width: data.width - 6, height: data.height - 6 }}>
        {isStart ? (<Play size={10} strokeWidth={2.25} className="text-emerald-500" fill="currentColor"/>) : (<Square size={10} strokeWidth={2.25} className="text-[var(--color-fg)]" fill="currentColor"/>)}
      </div>
