...
export function TaskNode({ data, selected }) {
    const { task, bindingState, bindingLabel, assigneeLabel, confidence, confidenceTier, evidence, } = data;
    ...
    const confidenceTitle = confidence != null
        ? `Confidence ${confidencePct}% · ${TIER_LABEL[confidenceTier]}${evidence ? ` — "${evidence}"` : ""}`
        : "No confidence signal";
    return (<div className={`group relative bg-[var(--color-surface)] border rounded-lg transition-all cursor-grab active:cursor-grabbing overflow-hidden ${selected
            ? "border-[var(--color-brand)] shadow-[0_0_0_3px_var(--color-accent-bg)]"
            : "border-[var(--color-border)] shadow-stripe-ambient hover:shadow-stripe-standard hover:border-[var(--color-border-strong)]"}`} style={{ width: data.width, height: data.height }}>
      ...
      <div className="flex flex-col h-full px-3 pt-2.5 pb-2.5">
        <div className="flex items-center justify-between gap-2">
          <span className={`inline-flex items-center gap-1 text-[9.5px] uppercase tracking-[0.12em] px-1.5 py-0.5 rounded border ${TYPE_BADGE[task.type]}`} style={{ fontWeight: 500 }}>
            <Icon size={9} strokeWidth={2.25}/>
            {typeLabel}
          </span>
          <div className="flex items-center gap-1.5 min-w-0">
            {/* Confidence dot — filled = high, half = medium, hollow = low */}
            {confidence != null && (<span title={confidenceTitle} className={`shrink-0 size-[7px] rounded-full ${confidenceTier === "high"
                ? TIER_COLOR.high
                : confidenceTier === "medium"
                    ? `${TIER_COLOR.medium} opacity-70`
                    : `border ${TIER_TEXT.low} border-current bg-transparent`}`}/>)}
            {assigneeLabel && (<span className="text-[10.5px] text-[var(--color-fg-muted)] font-mono truncate max-w-[11ch]" title={assigneeLabel}>
                {assigneeLabel}
              </span>)}
          </div>
        </div>

        <div className="mt-1 text-[13px] text-[var(--color-fg)] truncate leading-snug" style={{ fontWeight: 400 }} title={task.name}>
          {task.name}
        </div>

        <div className="mt-auto">
          {bindingLabel ? (<span className={`inline-block text-[10.5px] px-1.5 py-0.5 rounded border font-mono truncate max-w-full ${STATE_CHIP[bindingState]}`} title={bindingLabel}>
              {bindingLabel}
            </span>) : (<span className="text-[10.5px] text-[var(--color-fg-subtle)] italic">unbound</span>)}
        </div>
      </div>

      {/* Bottom confidence bar — width is confidence * card width. Lets
            the user judge certainty at a glance across many nodes. */}
      {confidence != null && (<div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[var(--color-surface-2)]" aria-hidden="true">
          <div className={`h-full transition-[width] duration-300 ${TIER_COLOR[confidenceTier]}`} style={{ width: barWidth }} title={confidenceTitle}/>
        </div>)}
