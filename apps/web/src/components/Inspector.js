...
import { TIER_COLOR, TIER_LABEL, TIER_TEXT, taskConfidence, tierOf, } from "../lib/confidence";
export function Inspector({ workflow, selectedTaskId, is, onClose }) {
    if (!workflow || !selectedTaskId)
        return null;
    ...
    const capabilityValid = system && binding?.capability ? system.capabilities.includes(binding.capability) : false;
    ...
    return (<motion.aside key={task.id} initial={{ x: 40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 40, opacity: 0 }} transition={{ type: "spring", stiffness: 320, damping: 30 }} className="w-80 shrink-0 bg-[var(--color-surface)] border-l border-[var(--color-border)] flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto nice-scroll px-4 py-3 space-y-4 text-xs">
        ...
        {actor && (<Field label="actor">
            <div className="flex flex-col gap-0.5">
              <span className="font-medium text-[var(--color-fg)]">{actor.name}</span>
              <code className="text-[11px] text-[var(--color-fg-muted)]">
                {actor.kind} · {actor.id}
              </code>
              {actor.is_ref?.user_id && (<code className="text-[11px] text-emerald-600 dark:text-emerald-400">is_ref.user_id = {actor.is_ref.user_id}</code>)}
              {actor.is_ref?.group_id && (<code className="text-[11px] text-emerald-600 dark:text-emerald-400">is_ref.group_id = {actor.is_ref.group_id}</code>)}
            </div>
          </Field>)}

        <Field label="binding">
          {!binding && <span className="italic text-[var(--color-fg-subtle)]">unbound — will compile to a human stub</span>}
          {binding && (<div className="flex flex-col gap-1.5">
              {binding.assignee_user_id && (<Provenance ok={!!assignee} reference={binding.assignee_user_id} kind="assignee" resolved={assignee ? `${assignee.name} (${assignee.id})` : "NOT FOUND"}/>)}
              {binding.candidate_group_id && (<Provenance ok={!!group} reference={binding.candidate_group_id} kind="group" resolved={group ? `${group.name} (${group.id})` : "NOT FOUND"}/>)}
              {binding.system_ref && (<>
                  <Provenance ok={!!system} reference={binding.system_ref} kind="system" resolved={system ? `${system.id} · ${system.kind}` : "NOT FOUND"}/>
                  {binding.capability && (<Provenance ok={capabilityValid} reference={binding.capability} kind="capability" resolved={capabilityValid
                        ? `declared by ${system?.id}`
                        : system
                            ? `${system.id} does not declare this`
                            : "system unknown"}/>)}
                </>)}
              {binding.form_key && (<Provenance ok={!!is?.deployed_forms?.some((f) => f.form_key === binding.form_key)} reference={binding.form_key} kind="form_key" resolved={is?.deployed_forms?.some((f) => f.form_key === binding.form_key)
                    ? "deployed on engine"
                    : "not deployed"}/>)}
              {binding.params && Object.keys(binding.params).length > 0 && (<div className="mt-1">
        ...
        {form && (<Field label="form">
            <div className="flex flex-col gap-0.5">
              ...
              <ul className="mt-1 space-y-0.5">
                {form.fields.map((f) => (<li key={f.id} className="text-[11px] text-[var(--color-fg-muted)] flex items-center gap-1.5">
...
function Provenance({ ok, reference, kind, resolved, }) {
    return (<div className={`border rounded-md px-2 py-1.5 ${ok ? "border-emerald-500/30 bg-emerald-500/10" : "border-rose-500/30 bg-rose-500/10"}`}>
...
function CompileHint({ task }) {
    const b = task.binding;
    if (task.type === "user") {
        const parts = [];
        if (b?.assignee_user_id)
            parts.push(`camunda:assignee="${b.assignee_user_id}"`);
        if (b?.candidate_group_id)
            parts.push(`camunda:candidateGroups="${b.candidate_group_id}"`);
        if (b?.form_key)
            parts.push(`camunda:formKey="${b.form_key}"`);
        return (<pre className="text-[11px] font-mono bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-md p-2 whitespace-pre-wrap text-[var(--color-fg)]">
  ...
  camunda:type="external"${topic ? `\n  camunda:topic="${topic}"` : ""} />`}
    ...
    return (<pre className="text-[11px] font-mono bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-md p-2 text-[var(--color-fg)]">
...
function WhyThis({ task }) {
    ...
    if (conf == null && !evidence)
        return null;
    return (<section className="rounded-md border border-[var(--color-border)] bg-[var(--color-surface-2)] px-3 py-2.5 space-y-2">
      <header className="flex items-center justify-between">
        ...
        {pct !== null && (<span className={`text-[10px] font-mono tnum ${TIER_TEXT[tier]}`}>
            {pct}% · {TIER_LABEL[tier]}
          </span>)}
      </header>

      {conf != null && (<div className="h-[4px] rounded-full bg-[var(--color-surface)] overflow-hidden" role="meter" aria-valuemin={0} aria-valuemax={100} aria-valuenow={pct ?? 0} title={`Extractor confidence ${pct}%`}>
          <div className={`h-full ${TIER_COLOR[tier]} transition-[width] duration-300`} style={{ width: `${conf * 100}%` }}/>
        </div>)}

      {evidence && (<blockquote className="text-[12px] leading-snug text-[var(--color-fg)] border-l-2 border-[var(--color-brand)] pl-2 italic">
