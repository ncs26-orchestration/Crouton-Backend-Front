...
import { ArrowRight, Bot, Check, HelpCircle, Loader2, Sparkles, X, Undo2, } from "lucide-react";
import { api } from "../lib/api";
import { collectLowConfidence, TIER_COLOR, TIER_TEXT } from "../lib/confidence";
import { useToasts } from "./Toasts";
let msgCounter = 0;
let patchCounter = 0;
export function CopilotPanel({ open, onClose, workflow, onIRUpdate, onHighlightTask, stage, interviewProgress, onFinalize, onItemResolved, onItemReopened, }) {
    ...
    const askMut = useMutation({
        mutationFn: (question) => api.copilotAsk({ ir: workflow, question }),
        onMutate: (question) => {
            const id = ++msgCounter;
            setMessages((prev) => [
                ...prev,
        ...
        onError: (err, _q, id) => {
            ...
            setMessages((prev) => prev.map((m) => (m.id === id ? { ...m, busy: false, error: message, text: "" } : m)));
        },
    });
    const clarifyMut = useMutation({
        mutationFn: (item) => api.copilotClarify({
            ir: workflow,
            kind: item.kind,
            element_id: item.id,
    ...
    return (<AnimatePresence>
      {open && (<motion.aside initial={{ x: 360, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 360, opacity: 0 }} transition={{ type: "spring", stiffness: 320, damping: 32 }} className="w-[360px] shrink-0 bg-[var(--color-surface)] border-l border-[var(--color-border)] flex flex-col h-full">
          <header className="px-4 py-3 border-b border-[var(--color-border)] flex items-center justify-between gap-2">
            ...
            <button onClick={onClose} className="text-[var(--color-fg-subtle)] hover:text-[var(--color-fg)]" aria-label="close copilot">
              <X size={14}/>
            </button>
          </header>
          ...
          {stage === "drafting" && interviewProgress && interviewProgress.total > 0 && (<div className="px-4 py-3 border-b border-[var(--color-border)] bg-[var(--color-accent-bg)]">
              ...
              <div className="mt-1.5 h-[4px] rounded-full bg-[var(--color-surface)] overflow-hidden">
                <motion.div className="h-full bg-[var(--color-brand)]" initial={false} animate={{
              ...
              <p className="mt-2 text-[11.5px] text-[var(--color-fg)] leading-snug" style={{ fontWeight: 400 }}>
                ...
                <button onClick={onFinalize} disabled={!onFinalize} className="underline underline-offset-2 decoration-dotted text-[var(--color-brand)] hover:decoration-solid disabled:opacity-50">
                  finalize as-is
                </button>{" "}
          ...
          <div className="px-3 pt-2 pb-2 border-b border-[var(--color-border)]">
            <nav className="flex gap-0.5 bg-[var(--color-surface-2)] rounded-md p-0.5">
              <ModeButton active={mode === "clarify"} onClick={() => setMode("clarify")} icon={<Sparkles size={11} strokeWidth={2}/>} label="Clarify" count={lowConfidence.length}/>
              <ModeButton active={mode === "ask"} onClick={() => setMode("ask")} icon={<HelpCircle size={11} strokeWidth={2}/>} label="Ask"/>
            </nav>
          </div>

          {/* Accepted patches ribbon */}
          {patches.length > 0 && (<div className="px-3 py-2 border-b border-[var(--color-border)] flex flex-wrap gap-1">
              {patches.map((p) => (<button key={p.id} onClick={() => undoPatch(p)} title={`Click to undo: ${p.label}`} className="group inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 hover:bg-rose-500/10 hover:border-rose-500/30 hover:text-rose-600 transition-colors">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto nice-scroll p-3 space-y-3 text-xs">
            {!workflow && (<div className="text-[11px] text-[var(--color-fg-subtle)] italic px-2">
                Extract a workflow first — the Copilot works on the current canvas.
              </div>)}

            {workflow && mode === "clarify" && (<>
                {lowConfidence.length === 0 && (<div className="text-[11px] text-[var(--color-fg-subtle)] italic px-2">
                    No low-confidence elements. Switch to Ask to query the workflow.
                  </div>)}
                {lowConfidence.map((item) => {
                    const itemKey = `${item.kind}:${item.id}`;
                    return (<ClarifyCard key={itemKey} item={item} applied={appliedItems.has(itemKey)} onFetchSuggestions={() => clarifyMut.mutateAsync(item)} onApply={(suggestion) => {
                            applyMut.mutate({ patch: suggestion.patch, label: suggestion.label }, {
                            ...
                            if (item.kind === "task" && onHighlightTask)
                                onHighlightTask(id);
                        }}/>);
                })}
              </>)}
          ...
          {mode === "ask" && (<form className="border-t border-[var(--color-border)] p-3" onSubmit={(e) => {
                    e.preventDefault();
                    submit();
                }}>
              <div className="flex items-end gap-2">
                ...
                }} rows={1} placeholder="Ask about actors, bindings, conditions…" disabled={!workflow || askMut.isPending} className="flex-1 min-w-0 resize-none bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-md px-2.5 py-1.5 text-[13px] text-[var(--color-fg)] placeholder:text-[var(--color-fg-subtle)] focus:outline-none focus:border-[var(--color-brand)] focus:bg-[var(--color-surface)] disabled:opacity-60" style={{ fontWeight: 300, maxHeight: "140px" }}/>
                <button type="submit" disabled={!workflow || !input.trim() || askMut.isPending} className="shrink-0 size-8 inline-flex items-center justify-center rounded-md bg-[var(--color-brand)] text-white hover:brightness-110 disabled:opacity-40" aria-label="Send">
...
function ModeButton({ active, onClick, icon, label, count, }) {
    return (<button onClick={onClick} className={`flex-1 flex items-center justify-center gap-1.5 rounded px-2 py-1 text-[11px] transition-colors ${active
            ? "bg-[var(--color-surface)] text-[var(--color-fg)] shadow-stripe-ambient"
            : "text-[var(--color-fg-muted)] hover:text-[var(--color-fg)]"}`} style={{ fontWeight: 400 }}>
      {icon}
      <span>{label}</span>
      {count !== undefined && count > 0 && (<span className="font-mono text-[10px] text-[var(--color-fg-subtle)] tnum">{count}</span>)}
    </button>);
}
function ChatBubble({ message }) {
    ...
    if (message.busy) {
        return (<div className="flex items-center gap-2 text-[var(--color-fg-subtle)]">
        <Loader2 size={12} className="animate-spin"/>
        <span className="text-[11px] italic">thinking…</span>
      </div>);
    }
    if (message.error) {
        return (<div className="rounded-lg border border-rose-500/30 bg-rose-500/10 px-2.5 py-1.5 text-[12px] text-rose-600 dark:text-rose-300">
        {message.error}
      </div>);
    }
    return (<div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-2)] px-2.5 py-2 text-[12px] text-[var(--color-fg)] leading-snug space-y-1.5">
      <p>{message.text}</p>
      {message.evidence && message.evidence.length > 0 && (<div className="flex flex-wrap gap-1 pt-1">
          {message.evidence.map((e, i) => e.ir_ref || e.quote ? (<span key={i} className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-fg-muted)] font-mono" title={e.quote}>
                {e.ir_ref ?? "?"}
...
function ClarifyCard({ item, applied, onFetchSuggestions, onApply, onKeepAsIs, onReopen, onHighlight, }) {
    ...
    if (applied) {
        return (<article className="rounded-md border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 flex items-center gap-2" onMouseEnter={() => onHighlight(item.id)} onMouseLeave={() => onHighlight(null)}>
        <Check size={12} className="text-emerald-500 shrink-0"/>
        <div className="flex-1 min-w-0">
          <div className="text-[10px] uppercase tracking-[0.12em] text-[var(--color-fg-subtle)]" style={{ fontWeight: 500 }}>
            {item.kind} · resolved
          </div>
        ...
        <button onClick={onReopen} className="text-[10px] font-mono text-[var(--color-fg-muted)] hover:text-[var(--color-brand)] transition-colors shrink-0" style={{ fontWeight: 500 }}>
          reopen
        </button>
      </article>);
    }
    return (<article className="rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden" onMouseEnter={() => onHighlight(item.id)} onMouseLeave={() => onHighlight(null)}>
      <button onClick={openAndFetch} className="w-full text-left px-3 py-2 flex items-start gap-2 hover:bg-[var(--color-surface-2)] transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.evidence && (<div className="text-[11px] text-[var(--color-fg-muted)] italic truncate mt-0.5">
              “{item.evidence}”
            </div>)}
        </div>
      </button>
      <AnimatePresence>
        {opened && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-[var(--color-border)] bg-[var(--color-surface-2)]">
            <div className="p-2 space-y-1.5">
              {loading && (<div className="flex items-center gap-2 text-[11px] text-[var(--color-fg-subtle)]">
                  <Loader2 size={11} className="animate-spin"/> thinking…
                </div>)}
              {error && (<div className="text-[11px] text-rose-600 dark:text-rose-400">{error}</div>)}
              {!loading && suggestions.length === 0 && !error && (<div className="text-[11px] text-[var(--color-fg-subtle)] italic">No suggestions available.</div>)}
              {suggestions.map((s, i) => (<button key={i} onClick={() => onApply(s)} disabled={!s.patch || s.patch.length === 0} className="w-full text-left rounded border border-[var(--color-border)] bg-[var(--color-surface)] px-2 py-1.5 hover:border-[var(--color-brand)] hover:bg-[var(--color-accent-bg)] transition-colors disabled:opacity-50 disabled:hover:border-[var(--color-border)] disabled:hover:bg-[var(--color-surface)]">
                  ...
                  {s.rationale && (<div className="mt-0.5 text-[11px] text-[var(--color-fg-muted)]">{s.rationale}</div>)}
                </button>))}
              {!loading && (<button onClick={onKeepAsIs} className="w-full text-left text-[11px] text-[var(--color-fg-muted)] italic px-2 py-1 hover:text-[var(--color-fg)] transition-colors">
...
function humanizeApplyError(raw) {
