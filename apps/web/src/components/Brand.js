...
export function BrandMark({ size = 20 }) {
    return (<svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
      ...
      <path d="M7 10l5-4 5 4" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M7 15l5-4 5 4" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity="0.7"/>
      <defs>
...
export function BrandWordmark({ className = "" }) {
    return (<span className={`font-display text-[var(--color-fg)] ${className}`} style={{ fontWeight: 300, letterSpacing: "-0.02em" }}>
      Pablo
    </span>);
}
