...
export function DecisionTableView({ table }: Props) {
  const vars = table.variables;
  return (
    <section className="rounded-md border border-[var(--color-border)] bg-[var(--color-surface-2)] overflow-hidden">
          ...
          <thead>
            <tr className="bg-[var(--color-surface)]">
              {vars.map((v) => (
                <th
                  key={v}
                  className="px-2 py-1.5 text-left font-mono text-[10.5px] text-[var(--color-fg-muted)] border-b border-[var(--color-border)]"
                  style={{ fontWeight: 500 }}
                >
                  {v}
                </th>
              ...
              <th
                className="px-2 py-1.5 text-left text-[10.5px] text-[var(--color-fg-muted)] border-b border-[var(--color-border)]"
                style={{ fontWeight: 500 }}
              >
                target
              </th>
              ...
              >
                {vars.map((v) => {
                  ...
                  return (
                    <td
                      key={v}
                      className="px-2 py-1.5 font-mono text-[var(--color-fg)] align-top"
                    >
                      {pred ?? (
                ...
                <td className="px-2 py-1.5 font-mono text-[var(--color-fg)] align-top">
                  <span className="inline-flex items-center gap-1">
                    ...
                    {rule.is_default && (
                      <span className="text-[9.5px] uppercase tracking-[0.12em] text-[var(--color-brand)]" style={{ fontWeight: 500 }}>
                        default
                      </span>
                    )}
                  </span>
      ...
      {table.evidence && table.evidence.length > 0 && (
        <div className="px-3 py-2 border-t border-[var(--color-border)] text-[10.5px] text-[var(--color-fg-muted)] italic space-y-0.5">
          {table.evidence.map((ev, i) => (
            <div key={i}>“{ev}”</div>
          ))}
        </div>
      )}
