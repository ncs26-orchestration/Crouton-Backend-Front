...
import { evidenceOf, taskConfidence, tierOf, } from "./confidence";
...
function isValidatingAgainstRegistry(is) {
    if (!is)
        return false;
...
function resolveBindingState(task, is) {
    if (!task.binding)
    ...
    if (task.type === "service") {
        if (!b.system_ref || !b.capability)
            return { state: "idle", label: "" };
        const sys = is?.systems.find((x) => x.id === b.system_ref);
        if (sys) {
...
export function irToFlow(wf, is, handlers) {
    const nodes = [];
