...
export function tierOf(c) {
    ...
    if (c >= CONFIDENCE_THRESHOLDS.high)
        return "high";
    if (c >= CONFIDENCE_THRESHOLDS.medium)
        return "medium";
    return "low";
}
...
export function taskConfidence(task) {
    const parts = [];
    if (task.confidence != null)
        parts.push(task.confidence);
    if (task.binding?.confidence != null)
        parts.push(task.binding.confidence);
    if (parts.length === 0)
        return undefined;
...
export function collectLowConfidence(wf) {
    if (!wf)
        return [];
    const out = [];
