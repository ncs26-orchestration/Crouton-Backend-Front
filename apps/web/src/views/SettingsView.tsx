...
import type { DeployTarget, Project } from "../lib/types";
...
export function SettingsView({
  ...
  const sortedProjects = [...projects].sort((a, b) => {
    if (scopedProjectId && a.id === scopedProjectId) return -1;
    if (scopedProjectId && b.id === scopedProjectId) return 1;
    return 0;
  });
    ...
    <div className="flex-1 overflow-y-auto nice-scroll bg-[var(--color-bg)]">
      <div className="max-w-3xl mx-auto px-8 py-10">
        ...
        <section className="mb-8">
          <h2
            className="text-[13px] text-[var(--color-fg)] flex items-center gap-2"
            style={{ fontWeight: 500 }}
          >
          ...
          <div className="mt-4 space-y-4">
            {projectsQuery.isLoading && (
              <div className="text-[12px] text-[var(--color-fg-subtle)] flex items-center gap-1.5">
                <Loader2 size={12} className="animate-spin" /> loading projects…
              </div>
            )}
            {!projectsQuery.isLoading && projects.length === 0 && (
              <div className="rounded-lg border border-dashed border-[var(--color-border-purple)] bg-[var(--color-accent-bg)] p-6 text-center">
            ...
            {sortedProjects.map((p) => (
              <ProjectTargetsCard
                key={p.id}
                project={p}
                initiallyOpen={p.id === scopedProjectId}
                onOpenOrgSettings={() => setOrgModalProject({ id: p.id, name: p.name })}
              />
            ))}
          </div>
        </section>
            ...
            <div className="mt-4">
              <button
                onClick={() => {
                  ...
                  if (proj) setOrgModalProject({ id: proj.id, name: proj.name });
                }}
        ...
        <section>
          ...
          <div className="mt-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4 flex items-center justify-between">
            ...
            <button
              onClick={toggle}
              className="inline-flex items-center gap-1.5 text-[12px] px-3 py-1.5 rounded-md border border-[var(--color-border)] bg-[var(--color-surface-2)] text-[var(--color-fg)] hover:border-[var(--color-border-strong)]"
              style={{ fontWeight: 500 }}
            >
...
function ProjectTargetsCard({
  ...
  const targetsQuery = useQuery({
    ...
    queryFn: () =>
      api.listDeployTargets(project.id).then((r) => r.deploy_targets),
  });
  const targets = targetsQuery.data ?? [];
  const [showForm, setShowForm] = useState(!!initiallyOpen && targets.length === 0);

  const [kind, setKind] = useState<"camunda7" | "elsa3">("camunda7");
  ...
  const createTarget = useMutation({
    mutationFn: (override?: {
      ...
      api.createDeployTarget(project.id, override ?? {
        kind,
        name: name.trim() || kind,
  ...
  const deleteTarget = useMutation({
    mutationFn: (id: string) => api.deleteDeployTarget(id),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["deploy-targets", project.id] }),
  });
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2">
          {onOpenOrgSettings && (
            <button
              ...
              className="text-[var(--color-fg-muted)] hover:text-[var(--color-brand)] p-1.5 rounded hover:bg-[var(--color-surface-2)]"
            >
              <Building2 size={14} />
            </button>
          )}
          <button
            ...
            className="inline-flex items-center gap-1 text-[11.5px] px-2.5 py-1 rounded-md border border-[var(--color-border)] bg-[var(--color-surface-2)] hover:border-[var(--color-border-strong)] transition-colors"
            style={{ fontWeight: 500 }}
          >
      ...
      {targets.map((t) => (
        <TargetRow key={t.id} target={t} onDelete={() => deleteTarget.mutate(t.id)} />
      ))}
        ...
        {LOCAL_DEPLOY_TARGETS.map((preset) => {
          ...
          return (
            <button
              key={preset.kind}
              type="button"
              onClick={() => createTarget.mutate(preset)}
              disabled={exists || createTarget.isPending}
              className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-md border border-[var(--color-border)] bg-[var(--color-surface-2)] text-[var(--color-fg)] hover:border-[var(--color-brand)] disabled:opacity-45 disabled:cursor-not-allowed"
              title={exists ? "Already registered for this project" : `Register ${preset.name} from docker compose`}
            >
              <Plus size={10} />
      ...
      {showForm && (
        <motion.form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            if (endpoint.trim()) createTarget.mutate(undefined);
          }}
        ...
        >
          <div className="grid grid-cols-2 gap-2">
            <Field label="Kind">
              <select
                value={kind}
                onChange={(e) => setKind(e.target.value as "camunda7" | "elsa3")}
                className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-md px-2 py-1 text-[12px] text-[var(--color-fg)]"
              >
            ...
            <Field label="Name">
              <input
                ...
                placeholder={kind === "camunda7" ? "Dev Camunda" : "Staging Elsa"}
                className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-md px-2 py-1 text-[12px] text-[var(--color-fg)] focus:outline-none focus:border-[var(--color-brand)]"
              />
            </Field>
          </div>
          <Field label="Endpoint">
            <input
              ...
              className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-md px-2 py-1 text-[12px] font-mono text-[var(--color-fg)] focus:outline-none focus:border-[var(--color-brand)]"
            />
          </Field>
          <div className="grid grid-cols-2 gap-2">
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            <button
              ...
              className="text-[11px] text-[var(--color-fg-muted)] hover:text-[var(--color-fg)] px-2 py-1"
            >
              cancel
            </button>
            <button
              type="submit"
              disabled={!endpoint.trim() || createTarget.isPending}
              className="inline-flex items-center gap-1 text-[11.5px] px-3 py-1.5 rounded-md bg-[var(--color-brand)] text-white hover:brightness-110 disabled:opacity-40"
            >
      ...
      {!showForm && targets.length === 0 && (
        <div className="px-4 py-4 text-[11.5px] text-[var(--color-fg-muted)] italic">
...
function TargetRow({ target, onDelete }: { target: DeployTarget; onDelete: () => void }) {
  const kindTone =
    target.kind === "camunda7"
      ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/30"
      : "bg-violet-500/15 text-violet-600 dark:text-violet-400 border-violet-500/30";
  return (
    <div className="group flex items-start gap-3 px-4 py-3 border-b border-[var(--color-border)] last:border-b-0 hover:bg-[var(--color-surface-2)] transition-colors">
      <span
        className={`inline-flex items-center gap-1 text-[10px] uppercase tracking-[0.12em] px-1.5 py-0.5 rounded border ${kindTone}`}
        style={{ fontWeight: 500 }}
      >
        {target.kind}
      </span>
...
function OrgSettingsModalWrapper({
  projectId,
  projectName,
  onClose,
}: {
  ...
  const overview = projectQuery.data?.project?.overview_json as any;
