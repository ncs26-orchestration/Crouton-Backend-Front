...
function loadLocation() {
    ...
    try {
        ...
        const validSections = ["home", "inbox", "workflows", "agents", "settings", "help"];
        return {
...
function AppRoot() {
    ...
    if (orgState === "none") {
        return (<OrgSetupView onDone={(org) => setOrgs([org])}/>);
    }
    const firstOrg = orgs[0];
    return (<OrgProvider initial={firstOrg}>
      <Shell />
    </OrgProvider>);
}
function Shell() {
    ...
    const openProject = (projectId) => setLocation({ section: "home", projectId, chatId: null });
    const openChat = (chat) => setLocation({ section: "home", projectId: chat.project_id, chatId: chat.id });
    const backToProjects = () => setLocation({ section: "home", projectId: null, chatId: null });
    ...
    return (<div className="h-screen w-screen overflow-hidden flex bg-[var(--color-bg)] text-[var(--color-fg)]">
      <ShellRail active={location.section} onSelect={setSection} onBrandClick={backToProjects} onUserClick={() => { qc.clear(); logout(); }}/>

      {(effectiveSection === "home") && (<>
          <ProjectTree selectedProjectId={location.projectId} selectedChatId={location.chatId} onOpenProject={openProject} onOpenChat={openChat} onBackToProjects={backToProjects} onOrganisationSettings={location.projectId ? openOrganisationSettings : undefined}/>
          {location.chatId ? (<ChatView chatId={location.chatId}/>) : (<ProjectsHomeView onOpenChat={openChat} onOpenProject={openProject}/>)}
        </>)}
      ...
      {effectiveSection === "settings" && (<SettingsView scopedProjectId={location.projectId}/>)}

      <HelpOverlay open={helpOpen} onClose={() => setHelpOpen(false)}/>

      {orgModalProject && (<OrgSettingsModalWrapper projectId={orgModalProject.id} onClose={() => setOrgModalProject(null)}/>)}
    </div>);
}
function OrgSettingsModalWrapper({ projectId, onClose }) {
    const [orgModalProject, setOrgModalProject] = useState(null);
    useEffect(() => {
    ...
    return (<OrgSettingsModal projectId={orgModalProject.id} projectName={orgModalProject.name} overview={null} onClose={onClose} onSaved={onClose}/>);
}
