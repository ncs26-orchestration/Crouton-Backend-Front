...
// marks the node completed (or blocked if the agent declared a dependency) with
// the agent's plain-language status (F5). Returns true if the node actually
// completed (not blocked) so the caller can track progress accurately.
